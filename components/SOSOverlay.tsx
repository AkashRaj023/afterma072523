...
const SOSOverlay: React.FC<SOSProps> = ({ profile, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-amber-400 flex items-center justify-center p-8 animate-in zoom-in duration-300">
        ...
        {!confirmed ? (
          <div className="space-y-6">
             <button 
                ...
                className="w-full py-6 bg-amber-400 text-black rounded-full text-xl font-black shadow-xl shadow-amber-100 hover:scale-[1.02] active:scale-95 transition-all"
             >
                YES, I NEED HELP NOW
             </button>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom duration-500">
             <div className="p-8 bg-amber-50 rounded-[2rem] border border-amber-100 flex items-center gap-6">
                ...
                <div>
                   <h4 className="font-bold text-amber-900">Emergency Alert Active</h4>
                   <p className="text-sm text-amber-700">SMS alert sent to your emergency contact: <span className="font-black underline">{profile.emergencyContact || 'Not Set'}</span></p>
                </div>
             </div>

             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <a href={`tel:${HELPLINES.emergency}`} className="p-8 rounded-[2rem] border-2 border-gray-100 flex flex-col items-center gap-3 group hover:border-amber-500 transition-all">
                   <Phone className="text-amber-500 group-hover:scale-110 transition-transform" size={32} />
                ...
                <a href={`tel:${HELPLINES.india.number}`} className="p-8 rounded-[2rem] border-2 border-gray-100 flex flex-col items-center gap-3 group hover:border-amber-500 transition-all">
